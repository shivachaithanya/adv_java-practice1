...
jQuery.fn.daterangepicker = function(settings){
	...
	var options = jQuery.extend({
		...
		prevLinkText: 'Prev',
		doneButtonText: 'Done',
		earliestDate: Date.parse('-1years'), //earliest date allowed 
...
//		dateFormat: 'dd/mm/yy', // date formatting. Available formats: http://docs.jquery.com/UI/Datepicker/%24.datepicker.formatDate
	...
	dateForm = options.dateFormat == "dd/mm/yy" ? "dmy" : "mdy";
//	dateForm = options.dateFormat == "dd/MM/yy" ? "dMMy" : "mdy";
...
//		var ul = jQuery('<ul class="ui-widget-content"></ul>').appendTo(rp);
		...
		/*ul.find('li').hover(
			...
			if(rangeInput.length == 2)
			{
				rangeInput.addClass('ui-daterangepicker-dateRange');
			}
			else
			{
				rangeInput.addClass('ui-daterangepicker-specificDate');
			}
	...
	function clickActions(el, rp, rpPickers, doneBtn){
		...
		if(el.is('.ui-daterangepicker-dateRange')){
			...
			rp.find('.range-start').restoreDateFromData().css('opacity',1).show(0);
			rp.find('.range-end').restoreDateFromData().css('opacity',1).show(0);
			setTimeout(function(){doneBtn.fadeIn();}, 0);
		}		
